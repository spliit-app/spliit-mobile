        ...
        {isInitialLoading ? (
          ...
          <ScrollView className="flex-1">
            <View className="p-4 mt-2 mb-4">
              <Text className="font-bold mb-0.5 text-lg text-foreground">
                Balances
              </Text>
                ...
                <View className="w-1/2 relative">
                  {balance.total !== 0 ? (
                    <View
                      className={
                        'h-full border ' +
                        (balance.total < 0
                          ? 'bg-background-negative border-border-negative self-end rounded-l-md'
                          : 'bg-background-positive border-border-positive self-start rounded-r-md')
                      }
                      style={{
                        width: `${Math.abs(
                  ...
                  <View className="px-2 absolute top-0 bottom-0 left-0 right-0 flex-col justify-center">
                    <Text
                      className={cn(
                        'text-foreground',
                        balance.total < 0 ? 'self-end' : 'self-start',
                      )}
                    >
                      {formatCurrency(
                        group.currency,
            ...
            {data.reimbursements.map((reimbursement, index) => {
              ...
              return (
                <View
                  key={index}
                  className="rounded-md mx-4 mb-2 p-4 bg-background-card"
                >
                  ...
                  >
                    <Text className="text-foreground-accent">Mark as paid</Text>
                  </Pressable>
                </View>
              )
            })}
